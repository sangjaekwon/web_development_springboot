...
/*
    1. 스픞링 부트 3 구조 살펴보기
        ...
        스프링 부트에서의 계층
            ...
            2. 비즈니스
                모든 비즈니스 로직을 처리
                    * 비즈니스 로직: 서비스를 만들기 위한 로직을 의마하며, 웹 사이트에서 벌어지는 모든 작업
                    -> 주문 서비스라고 하면 주문 개수, 가격 등의 데이터를 처리하기 위한 로직,
                    주문 처리를 하다가 발생하는 예외 처리 로직, 주문을 받거나 취소하는 등 프로세스를 구현하기 위한 로직 등
            3. 퍼시스턴스
                모든 데이터베이스 관련 로직을 처리. 이 과정에서 데이터베이스에 접근하는 DAO객체를 사용할 수도 있음
        ...
        스프링 부트 프로젝트 디렉토리 구성
            스프링 부트에는 정해진 구조가 없지만, 추천 프로젝트 구조가 있고, 이를 많은 개발자들이 따름
                main: 실제 코드를 작성하는 공간. 프로젝트 실행에 필요한 소스 코드나 리소스 파일은
                    모두 이 폴더 안에 들어있음(프로젝트 생성 시 자동 구현)
                test: 프로젝트의 소스 코드를 테스트할 목적의 코드나 리소스 파일이 들어있음
                build.gradle: 빌드를 설정하느 파일. 의존성이나 플러그인 설저오가 같이 빌드에 필요한 설정을 할 때 사용
